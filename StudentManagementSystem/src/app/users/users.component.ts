...
export class UsersComponent implements OnInit {
  ...
  getAll(pageNumber: number = 1) {
    ...
    return this.userService.getAll(this.request).then(
      (response: any) => {
  ...
  create(event?: any) {
    this.pageLoader.setLabel('Loading, please wait...');
    this.pageLoader.show();
    this.userService.showDialog(null, () =>
      this.getAll()
    );
  }
  ...
  edit(event?: any) {
    ...
    if (selectedRows.length > 1) {
      // tell me to select a single user
      this.notificationDialog.setLabel('Please select a user to continue.');
      this.notificationDialog.show();
    } else {
      if (selectedRows.length <= 0) {
        // tell me to select a user
        this.notificationDialog.setLabel(
          'Please select a user to continue.'
        );
        this.notificationDialog.show();
      } else {
        const selectedProductId = selectedRows[0];

        this.pageLoader.setLabel('Loading, please wait...');
        this.pageLoader.show();
        this.userService.get(selectedProductId).then(
          (response: any) => {
            this.pageLoader.hide();
            this.userService.showDialog(response, () =>
              this.getAll()
            );
          },
          (error: any) => {
  ...
  delete(event?: any) {
    ...
    if (selectedRows.length > 0) {
      ...
      this.confirmationDialog.setLabel(
        ...
        (selectedRows.length > 1 ? 'these users' : 'this user') +
        '? (Y/N)'
      );
      ...
      this.confirmationDialog.addEventHandler(() => {
        ...
        this.pageLoader.show();
        this.userService.delete(selectedRows).then(
          (response: any) => {
            this.pageLoader.hide();
            this.getAll();
            this.toastr.show({
      ...
      this.notificationDialog.setLabel(
        'Please select the user you want removed.'
      );
      this.notificationDialog.show();
    }
  }
  ...
  download(event?: any) {
    this.userService.downloadFileAsCSV().then(
      (response: any) => {
